...
data = pd.read_csv('kano.csv', usecols = [0,3,10,19], parse_dates=[0])
...
#ax.plot(data.date, data.PM25, color = 'red', lw = 4)
...
ax2.set_ylabel('Relative Humidity (%)', size = 29)
ax2.yaxis.set_tick_params(labelsize = 25)


bpatch = mpatches.Patch(color = 'red', label = '$PM_{2.5}$/$PM_{10}$')
rpatch = mpatches.Patch(color = 'steelblue', label = 'Rel. Humid.')

#Plot legend, title, and grid
plt.legend(handles=[bpatch,rpatch], fontsize = 25)


plt.title('Calabar PM Ratio vs Relative Humidity Rolling Mean', size = 33)


#plt.savefig('CalibarPM10Ratio.pdf')
plt.show()
